// Copyright 2026 Petri Koistinen. Licensed under the Apache License, Version 2.0.
  ...
  private enum RappApplePeerProfile {
    ...
    /// Deterministic CBOR for an empty map. Apple peer discovery currently
    /// needs no public parameter beyond its bound profile and candidate ID.
  ...
  internal final class RappPairingModel {
      ...
      internal func createOffer() {
        ...
        do {
          ...
          Task { [weak self] in
            ...
            guard let self, self.coordinator === coordinator, !self.isFinished
            else { return }
            relay.start()
          }
        } catch {
    ...
    internal func approvePeer() {
      guard case let .review(peer) = phase, let coordinator else { return }
  ...
  private extension RappPairingCoordinator.PairSummary {
    var remotePlatformLabel: String {
      switch role {
    ...
    var remotePlatformSymbol: String {
      switch role {
