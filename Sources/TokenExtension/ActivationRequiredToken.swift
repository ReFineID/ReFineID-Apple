...
internal final class ActivationRequiredToken: TKSmartCardToken, TKTokenDelegate {
  ...
  /// metadata, even when it has no keychain items. A benign empty
  /// session answers that query without pretending the card vanished.
  internal func createSession(_ token: TKToken) throws -> TKTokenSession {
